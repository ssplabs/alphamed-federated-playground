...
@router.post('/automl/task/create',
             summary="创建任务",
             status_code=status.HTTP_200_OK,
             response_model=response.TaskDetailResponse
             )
async def auto_ml_task_create(db:Session = Depends(get_database),
                              data: request.TaskCreateModel = Body(...)):
    """
    创建自动化建模任务

    """
    return_res, message = await services.create_task(db, data)
    _logger.info(f"service create task finish: {return_res}")
    if not return_res:
        return RetrieveResponse(content={}, code=-1,
                                message=f"task create failed! {message}")
    else:
        res_dict = return_res.to_dict()
        _logger.info(f"push message {res_dict}")
        return RetrieveResponse(content=jsonable_encoder(response.TaskDetailModel(**res_dict).dict()))
